...
const OwnerDashboard: React.FC<OwnerDashboardProps> = ({ user, onLogout, isConnected }) => {
  ...
  useEffect(() => {
    // Demo sessions for static deployment
    const demoSessions: Session[] = [
      {
        sessionId: "session_1_demo1",
        userName: "John Doe",
        isActive: true,
      ...
      <div className="flex-1 flex flex-col">
        {activeSession ? (
          <Chat
            ...
            role="owner"
            onMarkAsRead={() => {
              setSessions((prev) => prev.map((s) => (s.sessionId === activeSession ? { ...s, unreadCount: 0 } : s)))
            }}
          />
        ) : (
